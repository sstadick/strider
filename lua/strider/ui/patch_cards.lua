...
local function append_diff_blocks(out, card, lane)
	if not card.diff_blocks or #card.diff_blocks == 0 then
		return
	end
	table.insert(out, "")
...
local function folded_summary(status)
	if status == "success" then
		return "Patch complete — focus to expand"
	end
	if status == "error" then
		return "Patch failed — focus to expand"
	end
	if status == "cancelled" then
		return "Patch stopped — focus to expand"
	end
