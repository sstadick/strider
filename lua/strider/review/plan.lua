...
local function gaps_for_target(target, covered_ranges)
  ...
  table.sort(covered_ranges, function(a, b) return a[1] < b[1] end)
  ...
  for _, line in ipairs(vim.split(diff_text or "", "\n", { plain = true })) do
    ...
    if new_path and new_path ~= "/dev/null" then
      ...
      if hunk_start and current then
        ...
        if length == nil or length == 0 then length = 1 end
...
function M.plan_from_diff(cwd, base)
  if not cwd or not base or base == "" then return nil end
  ...
  for _, rel_path in ipairs(sorted_keys(per_file)) do
    ...
    for _, hunk in ipairs(hunks) do
      ...
      vim.list_extend(stops, chunk_range(cwd, abs_path, start_line, end_line, "diff", function(s, e)
...
local function find_anchor_line(path, needle, hint_line)
  if not path or not needle or needle == "" then return nil end
  local trimmed_needle = vim.trim(needle)
  if trimmed_needle == "" then return nil end
...
local function shifted_annotation(ann, offset)
  ...
  if offset ~= 0 then
    ...
    if start_line then start_line = start_line + offset end
    if end_line then end_line = end_line + offset end
  end
  return {
...
function M.normalize_stop(cwd, raw)
  if not raw or not raw.path or not raw.startLine or not raw.endLine then return nil end
  ...
  if not start_line or not end_line or start_line > end_line then return nil end
