...
local function card_lines()
  ...
  local draft = session and session.compose_buf and vim.api.nvim_buf_is_valid(session.compose_buf)
    and table.concat(vim.api.nvim_buf_get_lines(session.compose_buf, 0, -1, false), "\n") or ""
  local status = "Open :StriderChat to expand"
  if session and session.chat_card_status == "done" then status = "Chat done — :StriderChat to expand" end
  if session and session.last_summary and session.last_summary ~= "" then status = "Chat done — :StriderChat to expand" end
  if session and session.chat_card_status == "stopped" then status = "Chat stopped — :StriderChat to expand" end
  if session and session.last_error and session.last_error ~= "" then status = "Chat failed — :StriderChat to expand" end
  if running then status = "Chat running — :StriderChat to expand" end
  if vim.trim(draft) ~= "" then status = "Draft ready — :StriderChat to expand" end
  return { "› Strider chat", "────────────────────────────────", status }
end
...
local function refresh_flow_cards()
  ...
  if ok and flow_cards and flow_cards.refresh_layouts then flow_cards.refresh_layouts() end
end
...
local function attach(buf)
  if vim.b[buf].strider_chat_card_attached then return end
  vim.b[buf].strider_chat_card_attached = true
  local function open_chat()
    local ok, strider = pcall(require, "strider")
    if ok and strider and strider.chat then strider.chat() end
  end
  local function close_card() M.close() end
  ...
  for _, lhs in ipairs({ "q", "<Esc>" }) do
    vim.keymap.set("n", lhs, close_card, { buffer = buf, nowait = true, silent = true, desc = "Hide Strider chat card" })
  end
end
...
function M.close()
  ...
  if session then session.chat_card_win = nil end
  if was_visible then refresh_flow_cards() end
end

function M.refresh()
  local session = state.get_session("main")
  if not session or not session.chat_card_win or not vim.api.nvim_win_is_valid(session.chat_card_win) then return end
...
local function ensure_autocmds()
  ...
  vim.api.nvim_create_autocmd("VimResized", {
    group = augroup,
    callback = function() if M.is_visible() then M.open() end end,
  })
end
