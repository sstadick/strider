...
local function config_enabled()
  ...
  if config.log_pin_user_message == false then return false end
...
local function source_range(buf, message)
  if not message or not message.mark_id then return nil end
  local ok, mark = pcall(vim.api.nvim_buf_get_extmark_by_id,
    buf, source_namespace, message.mark_id, { details = true })
  if not ok or not mark or not mark[1] then return nil end
...
local function source_is_visible(win, buf, message)
  ...
  if not source_start then return false end
  local view_start, view_end = visible_range(win)
  if not view_start then return false end
...
function M.refresh_for_window(win)
  ...
  if not lane or not session then return end
...
function M.refresh_for_buffer(buf)
  if not lane_for_buffer(buf) then return end
...
function M.record_user_message(lane, text, buf, start_row, end_row)
  local session = state.get_session(lane)
  if not session or not buf or not vim.api.nvim_buf_is_valid(buf) then return end
