...
```text
Working (<elapsed>)                                      :StriderStop to interrupt
```
...
1. Update `lua/strider/ui.lua` in `compose_status_line()`.
   ...
   - Change the right side to only `:StriderStop to interrupt`.
...
3. Update comments in `lua/strider/ui.lua`.
...
4. Update tests in `tests/test_tmux_popups.py`.
   ...
   - Assert `:StriderStop to interrupt` is still present.
