...
local function size()
  ...
  return { col = math.max(info.width - width - 1, 0), compose_height = compose_height, log_height = log_height, width = width }
end
...
function M.open(on_send, deps)
  ...
  if session then session.chat_collapsed = false end
  chat_card.close()
