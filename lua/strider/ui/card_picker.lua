...
local function status_text(card)
  if card.status == "running" then return "running" end
  if card.status == "success" then return "done" end
  if card.status == "cancelled" then return "stopped" end
  if card.status == "error" then return "failed" end
  return card.status or "idle"
end

local function chat_status(session)
  if session.pending_request or session.progress then return "running" end
  if session.last_error and session.last_error ~= "" then return "failed" end
  local draft = ""
  ...
  if vim.trim(draft) ~= "" then return "draft" end
  if session.last_summary and session.last_summary ~= "" then return "done" end
  return session.chat_collapsed and "collapsed" or "open"
end

local function has_chat_card(session)
  if not session then return false end
  if session.pending_request or session.progress or session.chat_collapsed then return true end
  if session.last_summary or session.last_error then return true end
  if session.log_buf and vim.api.nvim_buf_is_valid(session.log_buf) then return true end
...
local function chat_item()
  ...
  if not has_chat_card(session) then return nil end
  return {
...
local function flow_items(opts)
  ...
  table.sort(items, function(a, b)
    if a.order == b.order then return a.rank < b.rank end
...
function M.items(opts)
  ...
  if not opts.kind then
    ...
    if chat then table.insert(items, chat) end
  end
