...
2. Keep the review API minimal: no new command; reuse `:SherpaNext` / `:SherpaNext!` for finish confirmation.
...
4. Better pending-turn controls: expose `:SherpaStop`, retry, steer, and clarify behavior while a turn is running.
...
10. Model and context affordance: provide a `:SherpaStatus` command for lane, model, thinking, context, pending request, and last error.
