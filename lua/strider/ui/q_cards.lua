...
local function ensure_turns(card)
	if card.turns and #card.turns > 0 then
		return card.turns
	end
	card.turns = {
		{
...
local function compact_status(card)
	...
	if has_body and done and status ~= "success" then
	...
	if done then
		if status ~= "success" then
			return "StriderQ stopped before an answer."
		end
...
local function append_turn(lines, turn, index)
	...
	if vim.trim(turn.answer_text or "") ~= "" then
		table.insert(lines, "")
	end
...
function M.start_turn(card, prompt)
	...
	card.answer_text = ""
	card.status = "running"
	card.started_at = turn.started_at
	if not card.prompt or card.prompt == "" then
		card.prompt = turn.prompt
	end
	return turn
end
...
function M.update_turn(card, text)
	local turn = current_turn(card)
	turn.answer_text = text or ""
	turn.status = "running"
	card.answer_text = turn.answer_text
	card.status = "running"
end
...
function M.finish_turn(card, text, status)
	...
	turn.status = status or "success"
	turn.finished_at = vim.uv.hrtime()
	card.answer_text = turn.answer_text or ""
	card.status = turn.status
	card.finished_at = turn.finished_at
end

function M.sync_legacy(session, card)
	if not session or not card or card.kind ~= "q" then
		return
	end
	if session.q_answer_card_id and session.q_answer_card_id ~= card.id then
		return
	end
	local turn = current_turn(card)
	...
	session.q_answer_prompt = turn.prompt or card.prompt or ""
	session.q_answer_text = turn.answer_text or ""
	session.q_answer_done = turn_status(turn) ~= "running"
	session.q_answer_status = turn_status(turn) == "running" and nil
		or (turn_status(turn) == "cancelled" and "error" or turn_status(turn))
end
...
function M.lines(card, expanded)
	...
	for index, turn in ipairs(turns) do
		...
		if index == 1 then
			question_count, separator_row = q_count, sep_row
		end
		if index == (card.current_turn_index or #turns) then
			body_row = row
		end
	end
